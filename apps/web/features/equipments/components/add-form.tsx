...
import { AssignmentInfo, BasicInfo, EquipmentForm, INITIAL_EQUIPMENT_FORM, INITIAL_FORM, PurchaseInfo, STEPS } from "@repo/types";
...
export function AddEquipmentPage() {
  ...
  function patchPurchase<K extends keyof PurchaseInfo>(key: K, value: PurchaseInfo[K]) {
    setForm((f) => ({ ...f, purchase: { ...f.purchase, [key]: value } }));
  }
  function patchAssignment<K extends keyof AssignmentInfo>(key: K, value: AssignmentInfo[K]) {
    setForm((f) => ({ ...f, assignment: { ...f.assignment, [key]: value } }));
  }

  if (submitted) {
    const icon = form.basic.category ? CATEGORY_ICONS[form.basic.category] : "📦";
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        <div className="bg-white rounded-3xl border border-slate-100 shadow-xl p-12 max-w-md w-full text-center space-y-5">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900">Equipment Added!</h2>
            <p className="text-slate-500 text-sm mt-1">
              <span className="font-semibold text-slate-800">{form.basic.name}</span> has been saved to your inventory.
            </p>
          </div>
          <div className="bg-slate-50 rounded-2xl p-4 text-left space-y-2">
            {[
              ...
              { label: "Assigned To",value: form.assignment.assignedTo || "Unassigned" },
          ...
          <div className="flex gap-3 pt-2">
            <button
              onClick={() => { setForm(INITIAL_EQUIPMENT_FORM); setStep(1); setSubmitted(false); }}
    ...
    <div className="min-h-screen bg-slate-50" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div className="max-w-3xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-center gap-3">
          ...
          {form.basic.name && (
            <div className="ml-auto flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 rounded-xl shadow-sm">
              <span className="text-base">{form.basic.category ? CATEGORY_ICONS[form.basic.category] : "📦"}</span>
              <span className="text-xs font-semibold text-slate-700 max-w-32 truncate">{form.basic.name}</span>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8">
          {step === 1 && !isReview && <StepBasic      data={form.basic}      onChange={patchBasic}      />}
          {step === 2 && !isReview && <StepPurchase   data={form.purchase}   onChange={patchPurchase}   />}
          {step === 3 && !isReview && <StepAssignment data={form.assignment} onChange={patchAssignment} equipmentStatus={form.purchase.status} />}
          {isReview                && <StepReview     form={form}                                       />}
        </div>

        <div className="flex items-center justify-between">
          <button
            onClick={() => isReview ? setStep(totalSteps) : setStep((s) => Math.max(1, s - 1))}
              ...
              return (
                <div
                  ...
                  className={`rounded-full transition-all duration-300 ${
                    dotStep === current   ? "w-5 h-2 bg-slate-900" :
                    dotStep < current    ? "w-2 h-2 bg-indigo-400" :
          ...
          {!isReview ? (
            <button
              onClick={() => {
                if (isLast) { setStep(totalSteps + 1); }
                else        { setStep((s) => s + 1); }
              }}
              disabled={!canNext}
